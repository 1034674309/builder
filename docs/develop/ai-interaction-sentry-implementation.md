...
| `internal/aiinteraction/failure.go` | 校验 completion/Archive 结果并生成统一的 `category`、`reason` 和 `request_id` |
| `internal/aiinteraction/detail_record.go` | 在 `http.server` 上记录原始 body，统一大小限制和截断信息 |
| `cmd/xbuilder-backend/aiinteraction_detail_middleware.go` | 捕获 AI endpoint 的原始 request/response body |
| `cmd/xbuilder-backend/util.go` | 把后端 `Failure` 复制到 HTTP 错误响应 |
...
- 失败分类和错误响应中的 `category` / `reason` 一致。
