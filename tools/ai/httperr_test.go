...
func TestErrorFromHTTPResponseBackendTags(t *testing.T) {
	body := `{"code":50000,"msg":"Internal server error","category":"upstream_failure","reason":"timeout","request_id":"req-1"}`
	err := ErrorFromHTTPResponse(500, 0, body, errors.New("upstream"))
	category, reason, requestID := backendTagsFromError(err)
	if category != "upstream_failure" || reason != "timeout" || requestID != "req-1" {
		t.Fatalf("backend tags = %q %q %q", category, reason, requestID)
	}
	wrapped := fmt.Errorf("ai interaction failed: %w", err)
	category, reason, requestID = backendTagsFromError(wrapped)
	if category != "upstream_failure" || reason != "timeout" || requestID != "req-1" {
		t.Fatalf("wrapped backend tags = %q %q %q", category, reason, requestID)
	}
}
...
func TestBackendTagsFromTimeoutWrappedHTTPError(t *testing.T) {
	...
	category, reason, requestID := backendTagsFromError(err)
	if category != "upstream_failure" || reason != "provider_error" || requestID != "abc" {
		t.Fatalf("backend tags = %q %q %q", category, reason, requestID)
	}
}
